...
export default function CRMPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormData({ company_name: '', phone_number: '', reception_date: '', business_type: '', status: '전자계약서' });
      fetchItems();
    } catch (error) {
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold text-white">원픽스파트너스 CRM</h1>
        ...
        <div className="bg-slate-800 rounded-lg p-6 mb-8 border border-slate-700">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            ...
            <button type="submit" className="col-span-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 rounded">
              추가하기
            </button>
          </form>
        </div>
        ...
        <div className="bg-slate-800 rounded-lg overflow-hidden border border-slate-700">
          <div className="overflow-x-auto">
            <table className="w-full text-white">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={6} className="px-6 py-4 text-center text-gray-400">로딩 중...</td></tr>
                ) : items.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-4 text-center text-gray-400">데이터가 없습니다</td></tr>
                ) : (
                  items.map((item) => (
                    <tr key={item.id} className="border-t border-slate-700 hover:bg-slate-700/50">
                      ...
                      <td className="px-6 py-4">
                        <span className="bg-blue-900 text-blue-100 px-3 py-1 rounded text-sm">{item.status}</span>
                      </td>
                      <td className="px-6 py-4">
                        <button onClick={() => handleDelete(item.id)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-sm">
                          삭제
                        </button>
                      </td>
                    </tr>
